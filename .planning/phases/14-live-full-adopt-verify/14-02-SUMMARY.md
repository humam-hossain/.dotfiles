...
- **Two runbook steps are wrong as written**, both worked around by the operator and both recorded for correction. Section 9's `stow -R kitty` from the repo root fails — packages live under `stow/`, and `arch/kitty.sh:10` uses `cd .../stow && stow -v=5 -t ~ kitty`; it also aborts on the pre-existing real `~/.config/kitty/kitty.conf`. Section 8's heredoc has an indented `SH` terminator, which a plain `<<` heredoc will not accept, so pasting it hangs the shell at `heredoc>`.
