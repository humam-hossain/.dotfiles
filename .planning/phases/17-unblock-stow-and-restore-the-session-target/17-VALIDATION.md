...
|-----------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 1a | FIX-01 | — | N/A | static grep | `! grep -rn -- '-v=5' arch/` | ❌ W0 | ⬜ pending |
| 1b | FIX-01, CAP-04 | — | N/A | static grep, counted | 15 sites carry `--verbose=5` and `--no-folding` | ❌ W0 | ⬜ pending |
| 1c | FIX-01 | — | N/A | syntax | `bash -n` over all 14 files | ❌ W0 | ⬜ pending |
| 1d | CAP-04 | — | N/A | static grep | `! grep -rn -- '-v=5' docs/` (recommended, §F-7) | ❌ W0 | ⬜ pending |
| 1e | FIX-01 | — | N/A | behavioral, non-mutating | `cd stow && stow --verbose=5 --no-folding -n -t ~ btop` → exit 0 | ❌ W0 | ⬜ pending |
| 2a | FIX-02 | — | Destructive `cp -rf` over a live config tree removed | static grep | `! grep -F 'cp -rf .config/hypr/' arch/hyprland.sh` | ❌ W0 | ⬜ pending |
| 2b | FIX-02 | — | N/A | static grep | `! grep -nE '(^\|[[:space:]])\.config/' arch/hyprland.sh` | ❌ W0 | ⬜ pending |
| 2c | FIX-02 | — | N/A | static grep | `grep -q 'HYPR-01' arch/hyprland.sh` | ❌ W0 | ⬜ pending |
| **2d** | FIX-02 | V4 access control (`sudo`, `usermod -aG i2c`) | Privilege change is transcripted and operator-acknowledged | **live, one-way** | OPERATOR — `script -c 'bash "$PWD/arch/hyprland.sh"' <transcript>` | ❌ manual-only | ⬜ pending |
| **2e** | FIX-02 | — | N/A | live | `hyprctl -j status \| jq -r .configProvider` → `lua`, parsed from the post-run transcript | ❌ semi-manual | ⬜ pending |
| 3a | FIX-04 | T-path-traversal | `safe_rm_path` refuses any existing path under the repo root | fixture, non-mutating | sourced-subshell loop over 3 existing repo paths, assert non-zero | ❌ W0 | ⬜ pending |
| 3b | FIX-04 | T-path-traversal | Pre-existing `$HOME` clause still refuses | fixture (negative control) | `safe_rm_path /etc/passwd` → non-zero | ❌ W0 | ⬜ pending |
| 3c | FIX-04 | T-path-traversal | Pre-existing hypr clause still refuses | fixture (negative control) | `safe_rm_path "$HOME/.config/hypr/custom"` → non-zero | ❌ W0 | ⬜ pending |
| 3d | FIX-04 | — | Wrapper is sourceable without executing `usage; exit 0` (D-09) | fixture | `( source arch/dots-hyprland.sh; type -t safe_rm_path )` → `function` | ❌ W0 | ⬜ pending |
| 4a | FIX-06 | — | N/A | static | `grep -Fxq '* text=auto eol=lf' .gitattributes` | ❌ W0 | ⬜ pending |
| 4b | FIX-06 | T-secret-in-vcs | Ignore patterns actually reach their targets (root-anchoring trap) | behavioral | `git check-ignore -v <path>` → exit 0, per pattern | ❌ W0 | ⬜ pending |
| 4c | FIX-06 | T-secret-in-vcs | Scan covers history **and** working tree, re-runnably | behavioral | `gitleaks detect --redact --exit-code 1` and `gitleaks dir . --redact --exit-code 1` | ❌ W0 | ⬜ pending |
| 4d | FIX-06 | T-secret-in-vcs | Every accepted finding carries a written reason (D-13) | static, conditional | each `[[rules.allowlist]]` / `[allowlist]` entry in `.gitleaks.toml` has a comment | ❌ W0 | ⬜ pending |
| 5a | START-02 | — | N/A | static grep (D-22) | `grep -Fq 'systemctl --user start hyprland-session.service' .config/hypr/custom/execs.lua` | ❌ W0 | ⬜ pending |
| 5b | START-02 | — | Hand-sync window between repo and live copy is closed each commit | behavioral (D-22) | `cmp -s .config/hypr/custom/execs.lua ~/.config/hypr/custom/execs.lua` | ❌ W0 | ⬜ pending |
| 5c | START-02 | T-disable-deletes-symlink | Unit stays `linked`, never `enabled` (D-17) | behavioral | `systemctl --user is-enabled hyprland-session.service` → `linked` | ❌ W0 | ⬜ pending |
| 5d | START-02 | — | N/A | behavioral, **mutating** | `systemctl --user start hyprland-session.service && systemctl --user is-active graphical-session.target` — run once by hand, keep out of the assert script | ❌ W0 | ⬜ pending |
| **5e** | START-02 | — | N/A | **manual-only (D-23)** | `systemctl --user is-active graphical-session.target` → `active` after a fresh login | ❌ manual-only | ⬜ pending |
| 6a | START-03 | T-disable-deletes-symlink | Footgun documented | static grep | `grep -q 'systemctl --user disable' docs/dots-hyprland-workflow.md` | ❌ W0 | ⬜ pending |
| 6b | START-03 | T-disable-deletes-symlink | Recovery command documented | static grep | `grep -Fq 'stow --verbose=5 --no-folding -t ~ systemd' docs/dots-hyprland-workflow.md` | ❌ W0 | ⬜ pending |
| 6c | START-03 | T-disable-deletes-symlink | `mask` named as the safe alternative | static grep | `grep -q 'systemctl --user mask' docs/dots-hyprland-workflow.md` | ❌ W0 | ⬜ pending |
...
- [ ] `sudo pacman -S --needed gitleaks` — prerequisite for 4c, no fallback. **Install from `extra`, never npm**: the npm package named `gitleaks` is `ycjcl868/gitleaks`, not `gitleaks/gitleaks`
- [ ] Operator decision on the tracked `.env` under `stow/` — blocks 4c's "zero findings" wording
- [ ] Pinned invocation form for `arch/hyprland.sh` recorded in `docs/dots-hyprland-workflow.md` — blocks 2d
- [ ] Update `docs/dots-hyprland-workflow.md:328,331` for the `FINDINGS=1 → 0` flip
- [ ] Read `scripts/phase13-d19-assert.sh`'s drift-pin logic before editing `arch/dots-hyprland.sh`
- [ ] Confirm gitleaks 8.30.1 sub-command spellings (`gitleaks --help`) before pinning 4c's commands
