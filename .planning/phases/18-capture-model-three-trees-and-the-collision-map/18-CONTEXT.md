...
**Gathered:** 2026-09-13
**Amended:** 2026-09-13 — post-discussion consistency review
**Status:** Ready for planning

> **Amendment note.** After the decisions were captured, all 61 were reviewed against the pinned installer source for internal conflicts and over-engineering. Seven conflicts, one counting error and three ambiguous cross-document decision references were found and corrected in place: D-03 (the inline rename at `3.files-legacy.sh:51-54` has no primitive token and was being silently dropped), D-04 and D-06 (the `$XDG_DATA_HOME` destinations and the fontconfig destination were excluded by wording that contradicted D-02's row count), D-05 (the `capture` tree value was underivable from an installer-scoped map), D-09 (`install_file__auto_backup` was tagged `cp-through` when it destroys the link, and bare `install_dir` had no tag at all — the tag is now derived from the outcome columns), D-11 and D-12 (a redundant `.gitkeep`; recovery text stated in three places), D-19 and D-20 (the phase adds three stow call sites, which would have made `scripts/phase17-unblock-assert.sh:90`'s `-eq 15` fail forever), D-22 and D-29 (`launch_first_available.sh`'s live copy is byte-identical to the vendored one, so "live wins" would have discarded the customised repo copy), and the MISC loop expansion (18 entries at the pin, not 17). Three of these were resolved by explicit user choice: hand-assigned `capture/` membership, full-legacy-script map coverage, and bumping the Phase 17 constant.
...
- **D-03:** Rows are produced by a generator that walks the vendored tree and emits `dest`, `primitive` and `source` mechanically. The `primitive → (symlink_outcome, repo_outcome)` mapping is a hand-authored lookup of roughly seven entries, sourced from `.planning/research/PITFALLS.md:285-290`. The assert regenerates the map and diffs it against the checked-in file, which means a pin bump that moves a row fails as a diff and satisfies criterion 3 without additional machinery. An unrecognised primitive fails loudly rather than defaulting to a safe-looking value. One destination is not reachable by primitive parsing at all and is special-cased by name: the inline `mv "${XDG_CONFIG_HOME}/hypr/hyprland.conf" "...hyprland.conf.old"` at `3.files-legacy.sh:51-54` is a bare rename with no source file and no primitive call, so the generator would silently omit it and the loud-failure rule would never fire. It emits a row with primitive `inline_rename`, outcome DESTROYED/untouched, and `3.files-legacy.sh:51-54` as its source. — **Reversibility:** costly — the generator's output format becomes the checked-in artifact that the assert, both tree READMEs and the derived tree placement all read; changing the column set later means regenerating the map, rewriting the assert's parser and re-deriving every package's placement.

- **D-04:** Coverage is every destination `3.files-legacy.sh` writes, whichever XDG base directory it lands in — which at the current pin means the `$XDG_CONFIG_HOME/fontconfig` row at line 41, the `$XDG_DATA_HOME/konsole` row at line 18, and the `$XDG_DATA_HOME/icons/illogical-impulse.svg` row at line 79 are all in the map. That is what makes D-02's ~28-row count reachable. `2.setups.sh` is excluded: it writes nothing capturable, its single link-touching line being `sudo ln -s` for a systemd unit outside `$HOME`. Font *package* installation (the `install_google_sans_flex` class in `3.files.sh`) and installer state are also excluded, but the fontconfig *destination* is not — an earlier wording conflated the two. The exclusion reasoning is recorded in the map header so a future reader does not have to rediscover it.

- **D-05:** The `tree` column is derived from the two outcome columns, never authored. Symlink preserved and repo untouched derives `stow`; symlink destroyed or repo overwritten derives `restow`. The column therefore has exactly two possible values. `capture` is *not* derivable from this map and never appears in the column: D-04 scopes coverage to the installer, and no installer primitive produces the rename-over-the-link behaviour that defines the tree. `capture/` membership is hand-assigned, stated as prose in `capture/README.md` — "a file belongs here when a non-installer writer renames over the link, the `switchwall.sh` and matugen class" — and `verify` checks `capture/` with the inverted expectation of D-50 without consulting the map at all. Phase 21, which brings the first inhabitant, may revisit whether those writers earn map rows of their own. The assert compares where a file actually sits against the derived value, for `stow/` and `restow/` only. — **Reversibility:** one-way — this is the property CAP-01 rests on. Allowing an authored override later would mean every reader must consult two sources to answer "how is this file captured", which is exactly the ambiguity the phase exists to remove.

- **D-06:** Paths are stored with their literal XDG variable — `$XDG_CONFIG_HOME/...` for most rows, `$XDG_DATA_HOME/...` for the two D-04 names — and expanded at read time. This keeps the map host-independent and makes each row diff cleanly against the source line it came from. There is no `SKIP_*` column: Phase 16 retired the safe profile, so every `SKIP_*` is false on our one install path. The reasoning is recorded in the header.
...
- **D-09:** The `rsync-replace` / `cp-through` tag is derived from the map's two *outcome* columns, not from the `primitive` column and not stored on disk. Symlink DESTROYED with the repo untouched derives `rsync-replace`, recovered by re-stowing; symlink preserved with the repo overwritten derives `cp-through`, recovered by `git checkout` and then re-stowing. Deriving from the outcomes rather than the primitive means the tag comes from the same source as the `tree` column of D-05 and a new primitive needs no second lookup table. It also fixes two errors an earlier primitive-keyed table carried: `install_file__auto_backup` renames the link away on firstrun (`3.files.sh:102` — `mv $t $t.old` then `cp_file`), so it is DESTROYED and `rsync-replace`, not `cp-through`; and bare `install_dir` (`3.files-legacy.sh:18`) was absent from the table entirely — it calls `rsync_dir`, whose `rsync -a` renames a new file over each link it overlaps, so it is DESTROYED and `rsync-replace` too, differing from `install_dir__sync` only in that it does not `--delete` the extras. Caveat worth stating in `restow/README.md`: `rsync-replace` is the vocabulary `.planning/ROADMAP.md:130` fixes for CAP-01, so it is kept verbatim even though two of its members (`install_file__auto_backup`, `inline_rename`) destroy the link with `mv` rather than rsync. The tag names the recovery, not the tool. `restow/README.md` carries a generated table listing each package with its tag and recovery command, in a region the generator owns. Same single-source-of-truth rule the `tree` column follows.
...
- **D-11:** Each tree gets a `README.md`. That README is what keeps an empty `capture/` alive in a clone, so no `.gitkeep` ships — an earlier draft of this decision added one, which was two files doing one job. No per-tree `.gitignore` ships this phase: Phase 17's D-14 wanted rules driven by the map, and the map says nothing about generated files yet. Phase 17's D-14 defers once more, to Phase 21.

- **D-12:** Each README has four parts — a one-paragraph contract stating what the installer does to files in that tree; the exact recovery command; the membership rule, stated as the map predicate for `stow/` and `restow/` and as D-05's hand-assigned prose rule for `capture/`, so a reader can place a new file without reading the map; and, for `restow/` only, the generated package-to-tag-to-recovery table. The contract prose is hand-written and reviewed; only the table is generated. The per-command recovery text appears once, in that generated table — `restow/README.md`'s prose and D-10's documented recovery both point at it rather than restating it, so a tag correction cannot leave a stale copy behind.
...
- **D-15:** Both folded directory symlinks found by Phase 17's D-02 audit — `~/.config/qBittorrent` and `~/.config/smartmontools` — are unfolded in this phase with `stow -D` followed by `stow --no-folding`. Every package then obeys `.planning/PROJECT.md`'s D-41 default and the assert needs no exception list. Neither package collides with the installer, so both stay in `stow/`.
...
- **D-19:** Neither package has a stow call site today — both were stowed by hand, so a fresh machine never links either one. Both are added: a new `arch/qbittorrent.sh`, and a stow line in `arch/scrutiny.sh`, each carrying the literal `--verbose=5 --no-folding` idiom. The call-site count asserted at `scripts/phase17-unblock-assert.sh:90` moves from 15 to 18 — two from this decision, one from D-08's second `arch/zsh.sh` invocation for starship — and D-20 authorises that edit explicitly.
...
- **D-20:** The `probe.sock` / `probe.socket` / `probe.lock` ignore patterns at `scripts/phase17-unblock-assert.sh:433-435` are left untouched. That script is a closed phase's verification record, and editing it retroactively would make the Phase 17 record describe something Phase 17 did not verify. Phase 18's own assert simply does not carry those patterns.
  ...
  One narrow exception is authorised, on the same grounds D-21 uses for the other two closed asserts: the literal `-eq 15` at line 90 becomes `-eq 18`, with a comment naming D-19 and D-08 as the reason. The principle D-20 protects is that a closed assert must not stop checking what its phase verified — and the property under test, *every* `arch/` stow call site carrying the literal `--verbose=5 --no-folding` pair, is unchanged; only the number of sites is. Leaving it at 15 would make the script fail forever, which destroys its value as a regression check far more thoroughly than bumping the constant does. A self-deriving count was rejected: rewriting the closed script's logic is a larger departure than changing its constant, and the counted-grep-as-complete-audit reasoning in the comment above line 89 would have to be rewritten too.
...
- **D-22:** Where the repo copy and the live file differ, live content wins — *unless* the live file is byte-identical to the vendored copy, in which case the repo content wins. The exception is not hypothetical; it decides one of the three drifted files. Measured during this review:

  | File | vendor | live | repo | Winner |
  |---|---|---|---|---|
  | `dolphinrc` | `14d27634` | `f236ddd7` | `217cc293` | live — all three differ, live is genuine KDE-written state |
  | `kdeglobals` | `c212651f` | `412a05b0` | `c360d020` | live — all three differ, live is genuine KDE-written state |
  | `hypr/hyprland/scripts/launch_first_available.sh` | `406185243dd4904d92fdd782c97f1213` | **identical to vendor** | `603181500b9bc190545d8441b3d38133` | **repo** — live was overwritten by the installer |

  `launch_first_available.sh` lives inside `hypr/hyprland/`, which `install_dir__sync` `--delete`s on every run, so its live bytes are upstream's and the repo copy is the customised one. Taking live there would repeat exactly the loss D-52 records for `starship.toml`. The redistribution table records the winner and the reason per file. The other seven files match their live counterpart byte for byte and move unchanged.
...
- **D-25:** `hypr` becomes two packages with the same name in different trees — `stow/hypr/` and `restow/hypr/`. `hypr/custom/` is `install_dir__ignore_existing` and safe; `hypr/hyprland/` is `install_dir__sync` and destroyed. The tree decides the meaning of the name. Noted for `restow/README.md`: package naming is not uniform — `restow/starship/` is named for the single file it carries, while `hypr` is a directory name duplicated across two trees. Nothing breaks today, since D-53 gives `verify` no arguments and no script takes a bare package name, but a future command that does would find `hypr` ambiguous.

- **D-26:** The map is generated and checked in *before* any file moves. `hypridle.conf` and `hyprlock.conf` are `install_file__auto_backup`, which on firstrun does `mv $t $t.old` then `cp -f` — renaming the symlink away, which is a destroyed outcome and puts them in `restow/`, not where intuition would place them. Every package boundary must be a mechanical consequence of the map. This is a hard internal ordering for the plan: map, then move, then re-stow, then verify. Phase 17's D-18 mandated `custom/execs.lua → stow/hypr/` row is safe either way, since `install_dir__ignore_existing` preserves the link.
...
**The redistribution table (FIX-03).** Twelve files leave repo-root `.config/`. Seven match their live counterpart byte for byte; three differ — two take live content and one takes repo content, per D-22's measured table; two are archived per D-23. Final tree placement for the `hypr/` rows is pending the map, per D-26 — the table below records the disposition decided in discussion, and the plan must reconcile the `stow/hypr/` rows against the generated map before executing the moves.

| Source path | Disposition | Destination | Note |
|---|---|---|---|
| `.config/dolphinrc` | move, live content wins (D-22) | `restow/` (MISC loop, `install_file` → cp-through) | repo copy was stale |
| `.config/kdeglobals` | move, live content wins (D-22) | `restow/` (MISC loop, `install_file` → cp-through) | repo copy was stale |
...
| `.config/hypr/hyprland/scripts/launch_first_available.sh` | move, **repo content wins** | `restow/hypr/` tagged `rsync-replace` | D-22 vendor-match exception; live is byte-identical to vendor. D-29; destroyed on every install until re-stowed |
...
**The MISC loop at the current pin** expands to 18 entries, which the generator must reproduce (counted against the pin during review; an earlier draft said 17 while listing 18): `chrome-flags.conf` (file), `code-flags.conf` (file), `darklyrc` (file), `dolphinrc` (file), `foot` (dir), `fuzzel` (dir), `kdeglobals` (file), `kde-material-you-colors` (dir), `kitty` (dir), `konsolerc` (file), `Kvantum` (dir), `matugen` (dir), `mpv` (dir), `starship.toml` (file), `thorium-flags.conf` (file), `wlogout` (dir), `xdg-desktop-portal` (dir), `zshrc.d` (dir).
