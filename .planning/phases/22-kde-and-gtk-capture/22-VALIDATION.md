...
| 22-02-01 | 02 | 2 | KDE-02 | T-22-05 | GTK per-file stowing under `stow/gtk/`, unfolded parent dirs (`~/.config/gtk-3.0`, `~/.config/gtk-4.0`), `gtk-dark.css` gitignored | integration | `test -L "$HOME/.config/gtk-3.0/settings.ini" && test -L "$HOME/.config/gtk-3.0/bookmarks" && test -L "$HOME/.config/gtk-4.0/settings.ini" && test -d "$HOME/.config/gtk-3.0" && test ! -L "$HOME/.config/gtk-3.0"` | ❌ W0 | ⬜ pending |
| 22-02-02 | 02 | 2 | KDE-02 | T-22-06 | GTK layout verification, unfolded parent directory assertion, and scratch link severance test | integration / drill | `./scripts/phase22-kde-and-gtk-capture-assert.sh --section 3` | ❌ W0 | ⬜ pending |
| 22-03-01 | 03 | 3 | KDE-02 | T-22-09 | GUARD list data file (`guard-paths.tsv`), Q7/Q8 measurement docs, `kdeglobals` retirement to `docs/archive/`, live unlinking | integration | `test -f guard-paths.tsv && test -f docs/archive/kdeglobals && test ! -e restow/kdeglobals && test ! -L "$HOME/.config/kdeglobals"` | ❌ W0 | ⬜ pending |
| 22-03-02 | 03 | 3 | KDE-02 | T-22-09 | `run_verify` GUARD check and sweep classifier refinement in `arch/dots-hyprland.sh` | integration / static | `./scripts/phase22-kde-and-gtk-capture-assert.sh --section 6` | ❌ W0 | ⬜ pending |
| 22-04-01 | 04 | 4 | KDE-03 | T-22-13 | `restow/chrome-flags/` packaging, collision map tag, `restow/README.md` generated table match | integration | `./scripts/phase22-kde-and-gtk-capture-assert.sh --section 4` | ❌ W0 | ⬜ pending |
| 22-04-02 | 04 | 4 | KDE-03 | T-22-14 | Live cp-through drill: clean-tree preflight, `install-files` write-through verification, `git checkout` recovery | live drill | `./scripts/phase22-kde-and-gtk-capture-assert.sh --section 5` | ❌ W0 | ⬜ pending |
| 22-04-03 | 04 | 4 | KDE-01, KDE-02, KDE-03 | T-22-15 | Overall tree integrity, zero folded ancestors, clean GUARD sweep, strict verify gate | system check | `./scripts/phase22-kde-and-gtk-capture-assert.sh && ./arch/dots-hyprland.sh verify --strict && ./scripts/phase17-unblock-assert.sh` | ✅ Exists | ⬜ pending |
