...
2. **Bootstrap Orchestrator Hardening (`bootstrap.sh`):**
   ...
   - **Fail-Soft Initial Theme Generation (D-12, D-13):** Created `generate_initial_theme()` in Step 6 to sanitize `.boxed-list row:insensitive` into `.boxed-list row:disabled` in `~/.config/matugen/templates/gtk-4.0/gtk.css`, check for wallpaper presence via `jq`, and fall back to `switchwall.sh --color "#3f51b5"` if wallpaper is missing or if theme generation encounters errors.
