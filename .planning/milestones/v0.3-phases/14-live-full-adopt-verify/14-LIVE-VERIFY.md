...
- *~~Runbook section 8's heredoc is unusable as printed.~~* **This recorded correction is itself wrong and is retracted (verification warning 1, cd5a68a).** No committed revision of `docs/phase14-adopt-runbook.md` contains a heredoc — `grep -cE "<<-?'?SH'?"` returns 0 against all three — and section 8 is a delegation by reference to `13-SOT-APPLY.md` §"Apply command (D-18)", which has no heredoc either. A paste failure did occur during the apply, but the artifact named as its cause did not contain the defect described, and the true cause was not captured. The operator ran the three named `cp -a` calls directly — same three files, same order, no delete-sync, `keybinds.lua`/`rules.lua`/`variables.lua` untouched. Verified: live and repo copies of `general.lua`, `env.lua` and `execs.lua` are byte-identical.
