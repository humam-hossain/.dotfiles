...
| `hyprland.conf` must-keeps — **monitors** (DP-1 / HDMI-A-2) | HIGH (subset) | migrate-to-hypr-custom | D-16 only: dual-monitor setup → `hypr/custom` overlay. Cite conf monitors ~29–30. **Migrated:** authored in Phase 13 as the `general.lua` overlay and applied live in the Phase 14 adopt — dual-head DP-1 + HDMI-A-2 verified live and byte-identical to the repo SoT. | full-profile / drop-skip-hyprland | 10-INVENTORY.md Axis A `hyprland.conf` + conf categories |
| `hyprland.conf` must-keeps — **workspaces** layout pins | HIGH (subset) | migrate-to-hypr-custom | D-16 only: workspace layout pins → `hypr/custom` overlay. Cite conf ~76–87. **Migrated:** authored in Phase 13 as eleven `hl.workspace_rule` pins in `general.lua` and applied live in the Phase 14 adopt. | full-profile / drop-skip-hyprland | 10-INVENTORY.md Axis A `hyprland.conf` + conf categories |
| `hyprland.conf` must-keeps — **env** (cursor + `ILLOGICAL_IMPULSE_VIRTUAL_ENV`) | HIGH (subset) | migrate-to-hypr-custom | D-16 only: machine env paths including cursor theme/size and `ILLOGICAL_IMPULSE_VIRTUAL_ENV` → `hypr/custom` overlay. Cite conf ~106–111. **Migrated as an empty slot:** Phase 13 D-21 and D-08 narrowed this row to no overlay content — upstream defaults match this machine — so `env.lua` went live in the Phase 14 adopt as a 1-byte require slot, with ii's own `hyprland/env.lua` supplying `ILLOGICAL_IMPULSE_VIRTUAL_ENV` and `hyprland/execs.lua` setting the cursor. | full-profile / drop-skip-hyprland | 10-INVENTORY.md Axis A `hyprland.conf` + conf categories |
...
**Archive policy (D-12):** `stow/waybar/.config/waybar`, `stow/rofi/.config/rofi` and `stow/swaync/.config/swaync` **remain in repo as archive** (pre-flight captures live). Success for Phases 11–14 does **not** delete these trees from the repo.
