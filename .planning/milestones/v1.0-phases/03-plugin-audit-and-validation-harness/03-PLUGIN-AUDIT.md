...
| 3rd/image.nvim | lua/plugins/neotree.lua | optional dep of neo-tree | none | libvips or similar image library (system) | Optional heavy dep; image support in file tree preview may be weak on Linux/Windows parity; degrades gracefully if missing | remove | Optional dependency of neo-tree; keeps image preview in tree; degrades gracefully when unavailable; actively maintained | Evaluate necessity in Phase 4; consider removing if not critical |
