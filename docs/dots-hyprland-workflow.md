...
> **Invocation form for `arch/hyprland.sh`.** Any invocation form works: `bash arch/hyprland.sh`, `./arch/hyprland.sh`, `bash "$PWD/arch/hyprland.sh"`, or `cd arch && ./hyprland.sh`. The script resolves one absolute `REPO_ROOT` at the top and reuses it at both stow stanzas. Before plan 17-06 it re-resolved `$(dirname "${BASH_SOURCE[0]}")` at each of its two directory changes, so the second one was evaluated from the directory the first had already moved to; the two repo-root-relative forms then aborted under `set -e` with `cd: arch/../stow: No such file or directory` — after the six package operations above them had already mutated the system. All four forms were re-measured after the fix and all four exit 0. The hoist is a deliberate, recorded departure from D-01 for this one file; it adopts the idiom `arch/waybar.sh` already uses. Assertion 2d in `scripts/phase17-unblock-assert.sh` holds it in place.
